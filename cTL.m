...
a=4.4;%Power law found from data fitting for the case of H2S
Btime=[200,600,1800,3600,14400,28800,86400];%time extrapolated to tmin and tmax
...
%cmax=Brho(1):
    TL=zeros(3,length(tt));
for k=1:3
    cmax=Brho(k,2)*((Btime(2)/Btime(1))^(1/a));
    Brho(k,1)=cmax;
    %cmin=Brho(7):
    cmin=Brho(k,6)*((Btime(6)/Btime(7))^(1/a));
    Brho(k,7)=cmin; 
        ...
        elseif ct(tstep)<cmin
            ...
            for i=2:length(Btime)
                if Brho(k,i-1)<ct(tstep)<Brho(k,i)
                    TL_rate=(1/Btime(i))*((ct(tstep)/Brho(k,i))^(a));%Toxic load rate in units of s^-1
                break;
                end
            end   
        end
